...
try {
    ...
    if ($previous instanceof 'Digistorm\Common\Exceptions\MaxRetriesException') {
        echo "Max retries!";
    }
}
----
...
[source,php]
----
$connectionPool = '\Digistorm\ConnectionPool\StaticNoPingConnectionPool';
$client = ClientBuilder::create()
...
[source,php]
----
$selector = '\Digistorm\ConnectionPool\Selectors\StickyRoundRobinSelector';
$client = ClientBuilder::create()
...
[source,php]
----
$serializer = '\Digistorm\Serializers\SmartSerializer';
$client = ClientBuilder::create()
